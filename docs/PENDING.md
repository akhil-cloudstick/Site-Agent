...
## Connected Sites (edit an external static site) — whole-site v2 done, follow-ups owed _(updated 2026-06-22)_
...
- [ ] **Real-project end-to-end test owed** — verified with a local fixture + the proven `deploy-cloudflare.ts`, but **not yet run against a real client `dist`/repo + real Cloudflare project**. Operator will provide a demo site (built to `docs/templateRule.md`); run connect → edit → publish → rollback against it. Also exercise the **repo-build path** (`npm install && npm run build`) on a real Astro repo (slow on the network drive — watch timeouts).
- [x] ~~**Replacement images break on the LIVE deploy (asset-bundle gap).**~~ **Done (2026-06-22):** on publish, uploaded images are copied into `<deploy>/sa-media/<file>` and the `src` repointed there, so they show on the live Cloudflare bundle (no `/api/media` left). Verified on a fixture (`publish.ts#bundleMedia`, reuses the block publisher's pattern). Original site images stay in the folder untouched.
- [ ] **Per-page content only** — shared components (nav/footer) edited on one page don't sync to others; each page's copy is independent. Consider a shared-content notion later.
- [ ] **Structural edits** (`m14-structural`) — add/remove sections & pages, design/code changes — later phase, kept separate from content editing.
