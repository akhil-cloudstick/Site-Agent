...
### Phase 2 structural — small follow-ups owed _(2026-06-25)_
- [ ] **One-click "Gallery" (and FAQ/pricing/logos) on the connected "+ Add section" palette** — today you add them by *describing* it to the AI; a labelled palette button is a small UX win. (Distinct from the **builder** typed-blocks below.)
- [ ] **Scripted e2e verify runner** `verify-structure-connected.ts` — exercise detect → move/delete/insert → remap + add/remove/reorder page on a fixture (the logic is covered by ~128 unit tests, but the manual runner the other `verify-*.ts` provide is missing).
- [ ] **Swap an inline-`<svg>` icon ("change icon")** — image logos use "Change image"; inline SVG icons can't be swapped via upload yet (would need SVG replacement). Social/icon links already get Set link · Move · Duplicate · Remove · Edit-with-AI.
- [ ] **Add a link to a *bare* unlinked image** — "Set link" works on elements already in a link/button; wrapping a standalone `<img>` in a link isn't wired (it's not in the link index).
...
- [ ] **More section types (builder typed-blocks)**: a gallery (multi-image), FAQ, pricing, logos as first-class **builder** blocks (structured fields) would round out common sites. _(This is the remaining unchecked Phase 2 item. On a **connected** site these can already be added via the AI "+ Add section" — see the connected follow-ups above.)_
