...
- [ ] **UI polish (remaining — cosmetic, not blocking):** top-bar styling + chat-section control layout. _(Progress/loading bar, cancel, and the ⋮ badge placement are done — see the two items above.)_
- [ ] **Progress hardening owed:** the cancel kill-tree path is implemented for win32 (`taskkill /T /F`) + posix (process-group) but **not yet verified on a live cancel** of a real clone/build/deploy; and the resumable job model assumes a single node (multi-node would need a shared queue, not the in-memory live registry).
